...
classDiagram
    ...
    Dealer --> DocStoreConnection : uses
    DocStoreConnection <|-- ESConn : implements
    DocStoreConnection <|-- InfinityConn : implements
    DocStoreConnection <|-- OBConn : implements
```
...
```mermaid
flowchart TD
    START([用户查询]) --> S1["1. 查询预处理<br/>分词 rag_tokenizer<br/>词权重计算 term_weight<br/>同义词扩展 synonym"]
    S1 --> S2["2. 查询构建<br/>全文查询: FulltextQueryer.question → MatchTextExpr<br/>向量查询: get_vector → MatchDenseExpr<br/>融合策略: FusionExpr weighted_sum"]
    S2 --> S3["3. 存储层检索<br/>DocStoreConnection.search<br/>ES/Infinity 执行混合检索<br/>返回 SearchResult"]
    S3 --> S4{"4. 重排序路由"}
    S4 -->|有 rerank_mdl| S4A[rerank_by_model]
    S4 -->|Infinity 引擎| S4B[直接使用 _score]
    S4 -->|ES 引擎| S4C["rerank 混合相似度<br/>tkweight * token_sim + vtweight * vector_sim"]
    S4A --> S5
    S4B --> S5
    S4C --> S5
    S5["5. 后处理<br/>排序特征增强 PageRank + 标签<br/>相似度阈值过滤<br/>分页提取<br/>聚合统计"] --> S6(["6. 返回结果<br/>total / chunks / doc_aggs"])
```
